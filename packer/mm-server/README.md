...
`magic_mirror_server` VM の作成・再作成は `scripts/apply-pve.sh -target=module.magic_mirror_server`
で行う。cloud-init スニペット（API キー等のシークレットを含む）は
`magic_mirror_server` を target に含めた場合だけローカルで生成・アップロードされる。
...
3. `scp` で pve-b550m の `local:snippets` にアップロード
