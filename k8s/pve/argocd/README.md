...
- `grafana-alloy` の hostPath volume は `controller.volumes.extra` / `alloy.mounts.extra`
  で設定すること (root の `extraVolumes` も `alloy.extraVolumeMounts` も alloy chart v0.12.x では機能しない)。
