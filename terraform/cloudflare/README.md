...
| `email.tf` | Email Routingの有効化とMX/SPF設定（個別routing ruleは必要時に追加） |
| `locals.tf` | published app、private route、internal DNS suffix の定義マップ |

### Email Routing

`miutaku.work` のCloudflare Email Routing有効化と、必要なMX/SPFレコードを `email.tf` で管理する。
現在は個別の受信用アドレス、転送先、Email Worker、routing ruleを管理していない。

受信用アドレスが必要になった場合は、用途に応じて `cloudflare_email_routing_rule` を
`email.tf` に追加する。転送先メールアドレスを使う場合は、Account単位の
`cloudflare_email_routing_address` を作成し、確認メールによる検証を完了してからruleを有効化する。
Email Workerを使う場合は、Workerを配備したうえでaction typeを `worker` にする。
...
| `tnlastation.miutaku.work` | rke2 | `http://tnlastation.app-tnlastation.svc.cluster.local:8080` | 必須 |
| `tnlastation-staging.miutaku.work` | rke2 | `http://tnlastation.app-tnlastation-staging.svc.cluster.local:8080` | 必須 |
