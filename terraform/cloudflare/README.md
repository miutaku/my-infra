...
| ホスト名 | バックエンド | Access |
|---|---|---|
| `argocd.miutaku.work` | `http://argocd-server.argocd.svc.cluster.local:80` | 必須 |
| `wol.miutaku.work` | `http://gptwol-service.app-gptwol.svc.cluster.local:5000` | 必須 |
| `unifi.miutaku.work` | `https://192.168.0.132:11443` | 必須 |
| `ix2215.miutaku.work` | `http://192.168.10.254` | 必須 |
| `nas-01.miutaku.work` | `https://192.168.20.191` | 必須 |
| `nas-02.miutaku.work` | `https://192.168.20.192` | 必須 |
| `pve-x570.miutaku.work` | `https://192.168.0.115:8006` | 必須 |
| `pve-b550m.miutaku.work` | `https://192.168.0.119:8006` | 必須 |
| `nanokvm-1.miutaku.work` | `http://192.168.10.240` | 必須 |
| `nanokvm-2.miutaku.work` | `http://192.168.10.241` | 必須 |

OKE 側は現時点では到達先なし。追加する場合も Cloudflare Access 前提の私用入口として
`locals.tf` の `oke_services` にエントリを追加して `terraform apply` する。
...
`dig argocd.miutaku.work` で Cloudflare の Anycast IP が返れば正常。
